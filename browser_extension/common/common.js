...
/**
 * Compares two manifest versions (see https://developer.chrome.com/extensions/manifest/version).
 *
 ...
 * </ul>
 *
 * @author http://stackoverflow.com/a/6832721
 */
function compareManifestVersions(v1, v2) {
    if (typeof v1 !== "string" || typeof v2 !== "string") {
        warnWithComponent("common", "Could not compare manifest versions because either is not a string: [$s] [$s]", v1, v2);
        return NaN;
    }
    ...
    if (!v1parts.every(isValidPart) || !v2parts.every(isValidPart)) {
        warnWithComponent("common", "Could not compare manifest versions because either is syntactically incorrect: [$s] [$s]", v1, v2);
        return NaN;
    }

    // Pad with zero parts if parts are not equally long
    // E.g. pad "1" to "1.0.0" if compared with "1.0.1"
